...
func (g *Game) drawStatus(screen tcell.Screen, origin boardOrigin) {
	...
	status += fmt.Sprintf("  FLAGS: %d/%d  TIME: %s", g.flagCount, mineCount, clock(g.runTime()))
	switch {
	case g.paused:
		status += " - " + pauseText
	case g.won:
	...
	hint := g.hint()
	emitStr(screen, centerX-len([]rune(hint))/2, origin.topY+boardRows+1, statusStyle, hint)
...
func (g *Game) hint() string {
	return core.JoinHints("Arrows/WASD: move", "Enter/X: reveal", "F: flag", g.keys.PauseHint(), g.keys.ResetHint("new field"), g.keys.LeaveHint())
}
...
func (g *Game) NeededSize() core.Size {
	status := "SCORE: 99999  BEST: 99999  FLAGS: 10/10  TIME: 99:59 - GAME OVER"
	return core.Size{
